...
from app.db.export_models import FacilityInventorySnapshot, SalesOrderRecord, ShopifyMasterData, SalesReturnRecord
...
class ReportsService:
    ...
    def _collect_garment_planning_rows(
        ...
        inventory_rows = (
            self.db.query(
                ...
                func.coalesce(func.sum(FacilityInventorySnapshot.available_inventory), 0).label("good_inventory"),
            )
            .filter(
    ...
    def production_planning_status_report(
        ...
        scanning_rows = (
            self.db.query(
                FacilityInventorySnapshot.sku.label("sku"),
                func.coalesce(func.sum(FacilityInventorySnapshot.available_inventory), 0).label("scanning"),
            )
            .filter(
                FacilityInventorySnapshot.sku.isnot(None),
                FacilityInventorySnapshot.sku != "",
